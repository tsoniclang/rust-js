...
pub(super) fn find(
    ...
    start: usize,
    sticky: bool,
) -> JsResult<Option<Match>> {
    if start > input.len() {
        return Ok(None);
    }
    if !input.is_char_boundary(start) {
        return Err(range_error(
            "RegExp index lies inside a native UTF-8 character",
        ));
    }
    ...
    let regex = compiled
        ...
        .map_err(Clone::clone)?;
    regex
        .try_find_from(input, start, super::super::execution_budget(input.len()))
...
pub(super) fn execute(expression: &JsRegExp, input: &str) -> JsResult<Option<Match>> {
    ...
    if stateful {
        expression.set_last_index(found.as_ref().map_or(0.0, |found| found.end() as f64));
    }
    Ok(found)
}
...
pub(super) fn build(expression: &JsRegExp, input: &str, matched: Match) -> RegExpExecArray {
    let values = super::super::array_from_optional(
        matched
            .groups()
            .map(|range| range.map(|span| input[span].to_owned()))
            .collect(),
    );
    let groups: BTreeMap<_, _> = matched
        .named_groups()
    ...
    let indices = expression.has_indices().then(|| {
        ...
        RegExpIndices {
            values,
            groups: (!named.is_empty()).then(|| RegExpNamedIndices {
                values: Rc::new(RefCell::new(named)),
            }),
        }
    });
    RegExpExecArray {
...
pub(super) fn collect(expression: &JsRegExp, input: &str) -> JsResult<Vec<Match>> {
    ...
    while let Some(found) = execute(expression, input)? {
        ...
        if empty {
            expression.set_last_index(advance(input, end) as f64);
        }
    }
    Ok(matches)
}

pub(super) fn append_substitution(
    output: &mut String,
    input: &str,
    matched: &Match,
    replacement: &str,
    ...
    while cursor < bytes.len() {
        ...
        match bytes.get(cursor).copied() {
            ...
            Some(b'&') => {
                output.push_str(&input[matched.range.clone()]);
                cursor += 1;
            }
            Some(b'`') => {
            ...
            Some(digit @ b'0'..=b'9') => {
                ...
                if let Some(next @ b'0'..=b'9') = bytes.get(cursor + 1).copied() {
                    ...
                    if pair > 0 && pair <= matched.captures.len() {
                        number = pair;
                        count = 2;
                    }
                }
                if number > 0 && number <= matched.captures.len() {
                    if let Some(range) = matched.group(number) {
                        output.push_str(&input[range]);
                    }
                    cursor += count;
                } else {

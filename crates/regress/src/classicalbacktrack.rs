...
struct ExecutionBudget {
    ...
    maximum_backtrack_frames: usize,
    exceeded: bool,
}
...
impl ExecutionBudget {
    fn unbounded() -> Self {
        Self {
            ...
            maximum_backtrack_frames: usize::MAX,
            exceeded: false,
        }
    }

    fn bounded(maximum_steps: u64) -> Self {
        Self {
            remaining_steps: Some(maximum_steps),
            maximum_backtrack_frames: usize::try_from(maximum_steps)
                .unwrap_or(usize::MAX)
                .min(1_048_576),
            exceeded: maximum_steps == 0,
        }
    }
    ...
    fn charge(&mut self, amount: usize) -> bool {
        ...
        self.remaining_steps = Some(remaining - amount);
        true
    }
}
...
impl<'a, Input: InputIndexer> MatchAttempter<'a, Input> {
    ...
    pub(crate) fn new_bounded(
        ...
        let mut result = Self::new(re, entry);
        result.budget = ExecutionBudget::bounded(maximum_steps);
        result
    }

    pub(crate) fn resource_limit_exceeded(&self) -> bool {
    ...
    fn run_lookaround<Dir: Direction>(
        ...
        let exceeds_frame_limit = match self.bts.len().checked_add(range.len()) {
            Some(length) => length > self.budget.maximum_backtrack_frames,
            None => true,
        };
        if !self.budget.charge(range.len().saturating_add(1)) || exceeds_frame_limit {
    ...
    fn try_at_pos<Dir: Direction>(
        ...
        'nextinsn: loop {
            'backtrack: loop {
                if self.bts.len() > self.budget.maximum_backtrack_frames || !self.budget.charge(1) {
                    self.budget.exceeded = true;
                    return None;
                }
                ...
                match re.insns.iat(ip) {
                    ...
                    &Insn::BackRef {
                        ...
                        if let Some(orig_range) = cg.as_range() {
                            if !self.budget.charge(orig_range.end - orig_range.start) {
                                return None;
                            }
                            if icase {
...
impl<Input: InputIndexer> BacktrackExecutor<'_, Input> {
    ...
    fn next_match_with_prefix_search<PrefixSearch: bytesearch::ByteSearcher>(
        ...
        let inp = self.input;
        loop {
            if !self.matcher.budget.charge(1) {
                *next_start = None;
                return None;
            }
            ...
            if self.matcher.resource_limit_exceeded() {
                *next_start = None;
                return None;
            }
...
impl<Input: InputIndexer> exec::MatchProducer for BacktrackExecutor<'_, Input> {
    ...
    fn resource_limit_exceeded(&self) -> bool {
        self.matcher.resource_limit_exceeded()
    }
}

...
mod tests {
    ...
    #[test]
    fn pieces_convert_to_corresponding_ir_nodes_in_order() {
        let nodes = Piece::into_nodes(vec![
            Piece::Char(b'a' as u32),
            Piece::ByteSequence(vec![b'b', b'c']),
            Piece::ByteSet(vec![b'D', b'd']),
            Piece::CharSet(vec![b'E' as u32, 0x212A]),
        ]);

        assert!(matches!(nodes[0], Node::Char { c } if c == b'a' as u32));
        assert!(matches!(&nodes[1], Node::ByteSequence(bytes) if bytes == b"bc"));
        assert!(matches!(&nodes[2], Node::ByteSet(bytes) if bytes == b"Dd"));
        assert!(matches!(&nodes[3], Node::CharSet(chars) if chars == &[b'E' as u32, 0x212A]));
    }
}

...
impl fmt::Display for ResourceLimitError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            formatter,
            "regular-expression execution exhausted its deterministic {}-step budget",
            self.maximum_steps,
        )
    }
}
...
impl Regex {
    ...
    /// Finds one UTF-16 match while enforcing an exact deterministic execution
    /// budget. Exhaustion is distinct from an ordinary no-match result.
    #[cfg(feature = "utf16")]
    pub fn try_find_from_utf16(
        &self,
        text: &[u16],
        start: usize,
        maximum_steps: u64,
        ...
        let mut matches = exec::Matches::new(
            super::classicalbacktrack::BacktrackExecutor::new(
                input,
                MatchAttempter::new_bounded(&self.cr, input.left_end(), maximum_steps),
            ),
            start,
        );
    ...
    /// Finds one UCS-2 match while enforcing an exact deterministic execution
    /// budget. Exhaustion is distinct from an ordinary no-match result.
    #[cfg(feature = "utf16")]
    pub fn try_find_from_ucs2(
        &self,
        text: &[u16],
        start: usize,
        maximum_steps: u64,
...
#[cfg(test)]
mod tests {
    use super::{Regex, backends};

    #[test]
    fn exact_input_encodings_and_ascii_backend_select_the_same_match() {
        let regex = Regex::new("a").unwrap();
        assert_eq!(regex.find_ascii("ba").unwrap().range(), 1..2);

        let backend_match =
            backends::find_ascii::<backends::DefaultExecutor<'_, '_>>(&regex, "ba", 0)
                .next()
                .unwrap();
        assert_eq!(backend_match.range(), 1..2);

        #[cfg(feature = "utf16")]
        {
            let input = "ba".encode_utf16().collect::<Vec<_>>();
            assert_eq!(
                regex.find_from_utf16(&input, 0).next().unwrap().range(),
                1..2,
            );
            assert_eq!(
                regex.find_from_ucs2(&input, 0).next().unwrap().range(),
                1..2,
            );
        }
    }

...
fn typed_array_copy_converts_values_without_aliasing_the_original_view() {
    ...
    assert_eq!(copy.get_number(0.0), Some(255));
    copy.set_number(1.0, 7.0);
    assert_eq!(original.get_number(2.0), Some(257));
...
fn typed_array_get_set_fill_and_slice() {
    ...
    assert_eq!(values.byte_length(), 8);
    assert_eq!(values.get_number(2.0), Some(3));
    values.set_number(1.0, 9.0);
    values.fill(7.0, -2.0, None);
    assert_eq!(values.get_number(1.0), Some(9));
    assert_eq!(values.get_number(2.0), Some(7));

    let copy = values.slice(1.0, Some(3.0));
    values.set_number(1.0, 100.0);
    assert_eq!(copy.get_number(0.0), Some(9));
}
...
fn typed_array_can_be_created_from_array_buffer() {
    ...
    assert_eq!(typed.get_number(0.0), Some(1));
    assert_eq!(typed.get_number(1.0), Some(2));
...
fn typed_array_subarray_is_shared_view() {
    ...
    assert_eq!(values.get_number(1.0), Some(99));
}
...
fn typed_array_set_accepts_exact_js_array_sources() {
    ...
    assert_eq!(values.get_number(0.0), Some(1));
    assert_eq!(values.get_number(1.0), Some(9));
    assert_eq!(values.get_number(2.0), Some(8));
    assert!(values
    ...
    assert_eq!(values.get_number(0.0), Some(7));
    assert_eq!(values.get_number(1.0), Some(5));
    assert_eq!(values.get_number(2.0), Some(3));
    assert_eq!(values.get_number(3.0), Some(2));
}
...
fn typed_array_view_constructors_subarrays_and_sorts_are_closed() {
    ...
    assert_eq!(offset.byte_offset(), 2);
    assert_eq!(offset.bytes_per_element(), 1);
    let bounded = Uint8Array::from_buffer_length(buffer, 2.0, 4.0).unwrap();
    bounded
        .set_from_fixed_array(&[4.0, 1.0, 3.0, 2.0], 0.0)
        .unwrap();

    assert_eq!(bounded.subarray_all().length(), 4);
    assert_eq!(bounded.subarray_from(1.0).length(), 3);

...
fn array_entries_keep_a_shared_live_cursor_and_stay_complete() {
    ...
    array.set(1, 9);
    assert_eq!(alias.next(), Some((1, 9)));
    array.push(11);
    assert_eq!(first.next(), Some((2, 0)));
    assert_eq!(alias.next(), Some((3, 11)));
...
fn array_entries_observe_shrinkage_and_empty_completion() {
    ...
    empty.push(5);
    assert_eq!(iterator.next_result().yield_value(), (0, 5));
}
...
fn dense_entry_projection_preserves_stored_absence_and_live_cursor() {
    ...
    assert_eq!(projected.next(), Some((0, None)));
    array.push(Some(11));
    assert_eq!(entries.next_result().yield_value(), (1, Some(7)));
    assert_eq!(projected.next(), Some((2, Some(11))));
...
fn dense_entry_projection_exposes_initialized_elements() {
    ...
    assert_eq!(array.entries().next(), Some((0, 0)));
}
